...
class User(models.Model):
    ...
    profile_image = models.ImageField(upload_to='users/', null=True, blank=True, default="")

    def __str__(self):
        return f"#{self.id} {self.name} {self.role} {self.is_verified}"

class BlogPost(models.Model):
    userId = models.ForeignKey(User, on_delete=models.CASCADE, related_name='blog_posts', default=1)
    title = models.CharField(max_length=200)
    image = models.ImageField(upload_to='blogs/', null=True, blank=True, default="")
    content = models.TextField()
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Blog {self.id} by {self.userId.name}"
...
class Product(models.Model):
    ...
    image = models.ImageField(upload_to='products/', null=True, blank=True, default="")
